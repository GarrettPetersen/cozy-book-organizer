...
The scene uses a fixed 1000 × 650 game world and scales it to the browser. This keeps physics deterministic and gives future scanned ink assets stable anchor points. Mobile landscape is the primary phone layout; portrait preserves the full room with letterboxing. Matter bodies drive loose books. Shelved books keep a canonical order and persistent gaps, while a small damped shelf simulation gives each spine an angle and angular velocity. Gaps let nearby books lean; inserting a supporting book changes their equilibrium so they settle upright again with visible inertia.
